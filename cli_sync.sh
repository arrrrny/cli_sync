...
show_help() {
  echo "Usage: $0 [sync|mcp|skills|status|backup|clear-backups] [--mcp|--skills]"
  echo ""
  echo "Commands:"
...
case "${1:-sync}" in
  sync)
    # Both MCP and Skills will be synced (default)
    ;;
  mcp)
    SYNC_SKILLS=false
    ;;
  skills)
    ...
    ;;
  status)
    # Run status for both MCP and Skills
  ...
  backup)
    ...
    ;;
  clear-backups|clear)
    # Clear backups for both MCP and Skills
